...
fun main() {
    maxKnapsack(arrayOf(1, 2, 3, 4), arrayOf(1, 2, 3, 4), 7).printList()
...
private fun maxKnapsackRecursive(
    ...
    if (weightArray[currentIndex] <= maxWeight) {
        take = valueArray[currentIndex] + maxKnapsackRecursive(weightArray, valueArray, maxWeight - weightArray[currentIndex], currentIndex + 1)
    }
...
private fun maxKnapsackDP(
    ...
    var take = Int.MIN_VALUE
    if (weightArray[currentIndex] <= maxWeight) {
        take = valueArray[currentIndex] + maxKnapsackDP(weightArray, valueArray, maxWeight - weightArray[currentIndex], currentIndex + 1, dpMap)
    }
    ...
    for (row in 1 until maxProfitMatrix.size) {
        for (column in 1 until maxProfitMatrix[0].size) {
            if (row > column) {
                ...
                maxProfitMatrix[row][column] = maxOf(valueArray[row - 1] + maxProfitMatrix[row - 1][column - weightArray[row - 1]], maxProfitMatrix[row - 1][column])
            }
        }
    }
...
private fun maxKnapsackTabulationOptimised(
    ...
    for (row in 1 until weightArray.size + 1) {
        for (column in maxWeight downTo  1) {
